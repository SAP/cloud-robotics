...
import (
	...
	"github.com/SAP/cloud-robotics/src/go/cmd/app-auth-proxy/internal/auth"
	appconfig "github.com/SAP/cloud-robotics/src/go/cmd/app-auth-proxy/internal/config"
...
func SetRoutes(router *mux.Router, appconfig *appconfig.Config, authOIDC *auth.OIDCConfig) {
	...
	for i := 0; i < len(appconfig.Routes); i++ {
		...
		forwardAuthToken := appconfig.Routes[i].ForwardAuthToken
		...
		if protected {
			router.PathPrefix(path).Handler(authOIDC.AuthN_Handler(authOIDC.AuthZ_Handler(group, serveFromProxy(target, path, removeFromPath, forwardAuthToken))))
		} else {
			router.PathPrefix(path).Handler(serveFromProxy(target, path, removeFromPath, forwardAuthToken))
		}

	}
}

func serveFromProxy(target string, path string, removeFromPath string, forwardAuthToken bool) http.HandlerFunc {
